...
mydata <- read.csv("~/coding/482/project/Actresses/actress_data_formatted.csv", header = TRUE, sep = ",", quote = "\"",
...
m <- glm(Winner ~ GGDramaWon + GGDramaLost + GGMusicalWon + GGMusicalLost + CCWon + CCLost + SAGWon + SAGLost + BAFTAWon + BAFTALost, family = binomial)
summary(m)
...
data.frame(Actress, pred.probs)
write.csv(data.frame(Actress, pred.probs), file = "./Actresses/probs_of_winning.csv")
